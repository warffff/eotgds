...
async function getMessageIdInfo() {
  // If DISCORD_STATUS_MESSAGE_ID is set, it is the source of truth.
  // This prevents Vercel from creating a new message on every update when Redis is empty/unavailable.
  const envId = CONFIG.discordMessageId();
  if (envId) {
    await kvSet(KEY_MESSAGE, envId);
    return { id: envId, fixed: true, source: 'env' };
  }

  const savedId = await kvGet(KEY_MESSAGE, '');
  if (savedId) return { id: savedId, fixed: false, source: 'redis' };

  return { id: '', fixed: false, source: 'none' };
}

async function upsertDiscordMessage(payload, chartPng) {
  const messageInfo = await getMessageIdInfo();

  if (messageInfo.id) {
    try {
      const edited = await editStatusMessage(messageInfo.id, payload, chartPng);
      return { message: edited, created: false, fixed: messageInfo.fixed, source: messageInfo.source };
    } catch (err) {
      // If the user explicitly set DISCORD_STATUS_MESSAGE_ID, never create duplicates silently.
      // Return the real error so the channel/message id can be fixed.
      if (messageInfo.fixed) {
        err.message = `Не удалось отредактировать фиксированное сообщение ${messageInfo.id}. Проверь DISCORD_STATUS_CHANNEL_ID, права бота и что сообщение находится именно в этом канале. Original: ${err.message}`;
        throw err;
      }
  ...
  return { message: created, created: true, fixed: false, source: 'created' };
}
...
export default async function handler(req, res) {
  try {
    ...
    return sendJson(res, 200, {
      ...
      fixed_message: !!result.fixed,
      message_source: result.source,
