...
export function buildDiscordPayload(status, history) {
  ...
  const isStale = Date.now() - Number(status.t || status.timestamp || Date.now()) > 60_000;
    ...
    embeds: [
      {
        ...
        footer: {
          text: `Автообновление каждые 15 секунд • Сегодня, в ${formatTodayTime()}`,
        },
      },
    ],
    components: [
